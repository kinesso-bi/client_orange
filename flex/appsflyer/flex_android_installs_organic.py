...
def report(date_target_start: date, date_target_end: date):
    ...
    sfx = '&timezone=Europe%2fWarsaw&additional_fields=device_model,keyword_id,store_reinstall,deeplink_url,oaid,install_app_store,gp_referrer,gp_click_time,gp_install_begin,amazon_aid,keyword_match_type,att,conversion_type,campaign_type,is_lat,device_category&maximum_rows=1000000'
    ...
    def insert_row(cursor, row):
        try:
            cursor.execute("""
                INSERT INTO Orange_Flex_Organic_Installs(
                        ...
                        %s,%s,%s,%s,%s,%s,%s,%s,%s
                           ...
                           (row[0], row[1], row[2], row[3], row[4], row[5],
                            ...
                            row[48], row[49], row[50], row[51], row[52], row[53],
                            row[54], row[55], row[56], row[57], row[58], row[59],
                            row[60], row[61], row[62], row[63], row[64], row[65],
                            row[66], row[67], row[68], row[69], row[70], row[71],
                            row[72], row[73], row[74], row[75], row[76], row[77],
                            row[78], row[79], row[80], row[81], row[82], row[83],
                            row[84], row[85], row[86], row[87], row[88], row[89],
    ...
    if data is None:
        ...
        if cursor is None:
            ...
            functions.db_disconnect(app_id, report_type, cnx, cursor, date_target_start=date_start,
                                    date_target_end=date_end, db_target="flex_android_installs_organic.py")
